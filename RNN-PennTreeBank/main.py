...
from utils import batchify, get_batch, repackage_hidden, depackage_hidden
...
from psgd import UVd, XMat
parser = argparse.ArgumentParser(description='PyTorch PennTreeBank RNN/LSTM Language Model')
parser.add_argument('--data', type=str, default='data/penn/',
                    help='location of the data corpus')
parser.add_argument('--model', type=str, default='GRU',
                    help='type of recurrent net (LSTM, QRNN, GRU, RNN)')
...
if args.resume:
    ...
    optimizer.lr_params = args.lr
    print(f'resuming lr {optimizer.lr_params}')
...
def train_loss(data,targets,_hidden):
    output, _hidden_last, rnn_hs, dropped_rnn_hs = model(data, _hidden, return_h=True)
...
def train():
    ...
    _hidden = model.init_hidden(args.batch_size)
    batch, i = 0, 0
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        lr2 = optimizer.lr_params
        # optimizer.param_groups[0]['lr'] = lr2 * seq_len / args.bptt
        optimizer.lr_params = lr2 * seq_len / args.bptt
        model.train()
        data, targets = get_batch(train_data, i, args, seq_len=seq_len)

        # Starting each batch, we detach the hidden state from how it was previously produced.
        # If we didn't, the model would try backpropagating all the way to start of the dataset.
        _hidden = repackage_hidden(_hidden)
        ...
        def closure():
            ...
            raw_loss = criterion(model.decoder.weight, model.decoder.bias, output, targets) + sum(
                [torch.sum(args.wdecay * torch.rand_like(param) * param * param) for param in model.parameters()]
            )
            ...
            return loss, _hidden_last
        # def closure():
        #     return train_loss(data,targets,_hidden)
        # `clip_grad_norm` helps prevent the exploding gradient problem in RNNs / LSTMs.
        if args.clip: torch.nn.utils.clip_grad_norm_(params, args.clip)
        loss, _hidden = optimizer.step(closure)
        total_loss += loss.data
        optimizer.lr_params = lr2

        if batch % args.log_interval == 0 and batch > 0:
            cur_loss = total_loss.item() / args.log_interval
            elapsed = time.time() - start_time
            print('| epoch {:3d} | {:5d}/{:5d} batches | lr {:05.5f} | precond lr {:05.5f} | ms/batch {:5.2f} | '
                    'loss {:5.2f} | ppl {:8.2f} | bpc {:8.3f}'.format(
                epoch, batch, len(train_data) // args.bptt, optimizer.lr_params, optimizer.lr_preconditioner,
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch in args.when:
            ...
            optimizer.lr_params /=2.
            # optimizer.lr_preconditioner /=2.
